...
namespace OldDragon
{
	namespace Itens
	{
		public class Inventario : Collection<Item>
		{
			...
			public IEnumerable<Protecao> ItensProtecaoUsando{ get{
				return this.OfType<Protecao>().Where((Item) => Item.Usando);
			}}
			public Inventario() : base() { }
			public Inventario(IEnumerable<Item> itens) : base() => AddRange(itens);
